...
class PipelinedealsPhpApi
{
    ...
    public function doRequest($res, $conditions = null, $page = null, $data = null)
    {
        ...
        if (!empty($conditions))
        {
            ...
            {
                $additional_params .= sprintf("&conditions[%s]=%s", $key, $value);
            }
        }

        // Adding page param to URL
        if (!empty($page)) {
            $additional_params .= sprintf("&page=%s", $page);
        }
    ...
    private function constructURL($res, $additional = null)
    {
        $this->request_url = sprintf('%s%s.json?api_key=%s%s', $this->base_url, $res, $this->api_key, $additional);
    }
